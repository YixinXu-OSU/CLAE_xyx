...
def merge(start: int, end: int, ref: bool, consensus_mode: str, fasta_src_name: str, lseq_enabled: bool, lseq_ref: bool):
    ...
    if lseq_enabled:
        if lseq_ref:
            ...
            fasta_name = 'results/linker/fasta/lseq_consensus_ref_' + consensus_mode + '_' + today + '_' + fasta_src_name
        else:
            file_prior = 'results/no_ref/Result_Lseq_sparc_'
            fasta_name = 'results/linker/fasta/lseq_consensus_nonref_' + consensus_mode + '_' + today + '_' + fasta_src_name
        total = pd.DataFrame()
        for i in range(start, end):
            total = pd.concat([total, pd.read_csv(file_prior + str(i) + '.csv').drop('Unnamed: 0', axis=1)])
        total.to_csv(file_prior + str(start) + '_' + str(end) + '_' + fasta_src_name + '.csv')
